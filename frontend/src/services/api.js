...
api.interceptors.response.use(r=>r, err=>{ if(err.response?.status===401){ localStorage.removeItem("token"); if(!["/login","/signup"].includes(window.location.pathname)) window.location.href="/login"; } return Promise.reject(err); });
export default api;
...
export const submitQuizWithAnswers  = (id,score,total_marks,user_answers)=>api.post(`/quizzes/${id}/submit`,{score,total_marks,user_answers});
export const generateQuiz          = (s,t,d="medium",n=5)=>{ if(s&&t&&!isNaN(s)&&!isNaN(t)) return api.post("/ai/generate-quiz",{subject_id:Number(s),topic_id:Number(t),difficulty:d,num_questions:n}); return api.post("/ai/generate-quiz",{subject_name:s,topic_name:t,difficulty:d,num_questions:n}); };
