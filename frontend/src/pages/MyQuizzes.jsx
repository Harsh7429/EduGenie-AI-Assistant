...
export default function MyQuizzes() {
  ...
  const handleRetry = (quiz) => {
    ...
    // Store quiz data for the GenerateQuiz page to pick up in "retake" mode
    sessionStorage.setItem("eg_retake", JSON.stringify({
      quiz_id: quiz.id,
  ...
  return (
    <div>
      <PageHeader title="Quiz" accent="History" accentColor="var(--jade)"
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {filtered.map((quiz, idx) => {
          ...
          return (
            <div key={quiz.id} className="glass fade-up" style={{ overflow:"hidden", animationDelay:`${idx * 0.04}s` }}>
              {/* Row header */}
              <div style={{ padding:"14px 18px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap", borderBottom: isExp ? "1px solid var(--border)" : "none" }}>
                ...
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button onClick={() => handleRetry(quiz)} style={{
                    padding:"5px 12px", borderRadius:7, cursor:"pointer",
                    border:"1px solid var(--jade-border)", background:"var(--jade-dim)",
                    color:"var(--jade)", fontFamily:"var(--font-body)", fontSize:".77rem", fontWeight:600, transition:"all .15s",
                  }}>Retry</button>
                    ...
                    {parsed.questions.map((q, qi) => (
                      <div key={qi} style={{ padding:14, borderRadius:10, background:"var(--bg-elevated)", border:"1px solid var(--border)" }}>
                        <p style={{ fontWeight:600, color:"var(--ink)", marginBottom:10, fontSize:".87rem", lineHeight:1.55 }}>
                          <span style={{ color:"var(--ink-3)", marginRight:8, fontFamily:"var(--font-mono)", fontSize:".72rem" }}>Q{qi + 1}</span>
                          {q.question}
                        </p>
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:5 }}>
                          {q.options.map((opt, oi) => (
                            <div key={oi} style={{
                              padding:"7px 11px", borderRadius:7, fontSize:".82rem",
                              background: oi === q.correct_answer ? "var(--jade-dim)" : "var(--bg-overlay)",
                              border:     oi === q.correct_answer ? "1px solid var(--jade-border)" : "1px solid var(--border)",
                              color:      oi === q.correct_answer ? "var(--jade)" : "var(--ink-3)",
                              ...
                              {oi === q.correct_answer && <span style={{ fontSize:".78rem", flexShrink:0 }}>✓</span>}
                            </div>
                          ))}
                        </div>
                        {q.explanation && (
