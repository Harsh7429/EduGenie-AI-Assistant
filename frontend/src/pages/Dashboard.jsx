  ...
  return (
    <div>
      ...
      <div className="fade-up" style={{ marginBottom: 40 }}>
        ...
        <h1 className="title-font" style={{ fontSize:"clamp(1.8rem,4vw,3rem)", lineHeight:1.1,
        ...
        <p style={{ color:"#64748b", fontSize:"1rem", maxWidth:480, lineHeight:1.6 }}>
          Your AI-powered MCA study companion. Master every topic with smart notes, adaptive quizzes, and real-time progress tracking.
        </p>
      </div>

      {/* Personal stat strip */}
      <div className="fade-up" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:40 }}>
        {[
          ...
          <div key={i} className="glass pulse-glow" style={{ padding:"20px", textAlign:"center" }}>
            <div style={{ fontSize:"1.6rem", marginBottom:6 }}>{s.icon}</div>
            <div className="title-font" style={{ fontSize:"1.8rem", color:s.color, lineHeight:1 }}>
              {loading ? "—" : s.value}
            </div>
            <div style={{ color:"#475569", fontSize:"0.72rem", marginTop:5, textTransform:"uppercase", letterSpacing:"0.06em" }}>
              {s.label}
            </div>
          </div>
        ))}
            ...
            {NAV_CARDS.map((card,i) => (
              <div key={card.path} className="fade-up" onClick={() => navigate(card.path)}
                style={{ background:`linear-gradient(135deg,${card.accent}18,${card.accent}06)`,
                  border:`1px solid ${card.accent}25`, borderRadius:14, padding:"22px 18px",
                ...
                <div style={{ fontWeight:700, fontSize:"0.9rem", color:"#f1f5f9", marginBottom:4 }}>{card.label}</div>
                <div style={{ color:"#475569", fontSize:"0.76rem", lineHeight:1.5 }}>{card.desc}</div>
                <div style={{ marginTop:14, color:card.accent, fontSize:"0.75rem", fontWeight:700 }}>Open →</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="glass fade-up" style={{ padding:"20px 28px", display:"flex", gap:32, alignItems:"center" }}>
        ...
        {[
          ...
          <div key={s.label} style={{ display:"flex", alignItems:"center", gap:8 }}>
            <span className="title-font" style={{ fontSize:"1.4rem", color:"#6366f1" }}>{loading ? "—" : s.value}</span>
