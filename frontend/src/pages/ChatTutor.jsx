...
const SendIcon = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    <path d="M14 8L2 2l2.5 6L2 14L14 8z" fill="currentColor"/>
  </svg>
);
...
export default function ChatTutor() {
  ...
  const clearChat = () => {
    setMessages([{ role:"assistant", content:"Chat cleared! Ask me anything about MCA topics." }]);
    toast.info("Chat cleared.");
  };

  const STARTERS = ["Explain binary search trees","What is normalization in DBMS?","How does TCP/IP work?","What is the OSI model?"];

  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 110px)", minHeight:480 }}>
      <style>{`@media(max-width:860px){.chat-outer{height:calc(100vh - 130px)!important;}}`}</style>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          <select className="glow-input" value={subject} onChange={e => setSubject(e.target.value)} style={{ width:180 }}>
            {SUBJECTS.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <button onClick={clearChat} style={{ padding:"8px 14px", borderRadius:8, border:"1px solid var(--border-med)", background:"transparent", color:"var(--ink-3)", fontFamily:"var(--font-body)", fontSize:".78rem", cursor:"pointer", whiteSpace:"nowrap", transition:"all .15s" }}
      ...
      {/* Messages */}
      <div className="glass chat-outer" style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:12, marginBottom:12 }}>
        {messages.length === 1 && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:7, marginBottom:4 }}>
            {STARTERS.map(s => (
              <button key={s} onClick={() => handleSend(s)} style={{
                padding:"6px 12px", borderRadius:99, fontSize:".77rem",
                border:"1px solid var(--border-med)", background:"var(--bg-elevated)",
                color:"var(--ink-3)", cursor:"pointer", fontFamily:"var(--font-body)", transition:"all .15s",
              }}
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{ display:"flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              ...
              background: msg.role === "user" ? "var(--sapphire)" : "var(--bg-3)",
            ...
            <div style={{ padding:"12px 16px", borderRadius:12, borderBottomLeftRadius:4, background:"var(--bg-3)", border:"1px solid var(--border)", display:"flex", gap:4, alignItems:"center" }}>
              {[0,1,2].map(i => (
                <div key={i} style={{
                  ...
                  animation:`bounce .8s ${i * 0.15}s ease-in-out infinite`,
                }}/>
              ))}
            </div>
          </div>
      ...
      <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
        <textarea
          ...
          rows={1}
          style={{
            flex:1, background:"var(--bg-3)", border:"1px solid var(--border-med)",
            borderRadius:11, padding:"11px 14px", color:"var(--ink)",
            fontFamily:"var(--font-body)", fontSize:".9rem", outline:"none",
            resize:"none", lineHeight:1.5, maxHeight:120, overflowY:"auto",
            transition:"border-color .18s", WebkitAppearance:"none",
          }}
        ...
        <button onClick={() => handleSend()} disabled={loading || !input.trim()}
          style={{
            ...
            background: input.trim() ? "var(--sapphire)" : "var(--bg-elevated)",
